...
def strip_poison(background, overlay):
    ...
    added_image = Image.blend(background,overlay,0.5)
    #added_image = added_image.resize((224,224))
    return added_image

def entropyCal(background, ds, model, num, feature_extractor):
    ...
    entropy_sum = [0] * num
    x1_add = [0] * num
    index_overlay = np.random.randint(3000,3999, size=num)
    py1_add = []
    for x in range(num):
        example = ds[int(index_overlay[x])]
        #print(example["image"])
        pixel_values = example["img"].convert("RGB")
        temp = _train_transforms(strip_poison(background, pixel_values))
        inputs = feature_extractor(images=temp, return_tensors="pt").to("cuda")
        temp_output = model(**inputs)
        #print(type(temp_output.logits))
        #print(temp_output.logits.shape)
        #print(temp_output.logits.tolist())
        #print(torch.nn.softmax(temp_output.logits).tolist())
        #print(-np.nansum(temp*temp))
        py1_add.append(torch.nn.functional.softmax(temp_output.logits, dim=-1).tolist())
    #print(py1_add)
    w = np.log2(py1_add)
    EntropySum = -np.nansum(py1_add*w)
    return EntropySum

def strip(ds, model, feature_extractor):
    num_sample = 1000
    num_test = 100
    entropy_benigh = [0] * num_test
    entropy_trojan = [0] * num_test
    for i in range(num_test):
        example = ds[i]
        x_background = example["img"].convert("RGB")
        entropy_benigh[i] = entropyCal(x_background, ds, model, num_sample, feature_extractor)
        print(entropy_benigh[i])
    #print(w)
    entropy_benigh = [x / num_sample for x in entropy_benigh]
    print("entropy_benigh : ", min(entropy_benigh))
...
def main():
    ...
    def train_transforms_backdoor(example_batch):
        ...
        if rand > 1 - data_args.backdoor_p:
            if data_args.poison_type=="BadNets":
                ...
                example_batch[label_name] = [0 for raw_label in example_batch[label_name]]
                example_batch[image_name] = [poison_img(raw_image) for raw_image in example_batch[image_name]]
    ...
    strip(ds["train"], model, feature_extractor)
