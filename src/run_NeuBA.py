...
from PIL import Image
from utils import poison_img, poison_text, std_poison_img, rotate_poison_img, toxic, read_labels, read_all_images
...
def get_force_features(dim=768, lo=-5, hi=5):
    force_features = []
    for i in range(0, 197):
        #or j in range(i, 4):
        dim_div_192 = dim // 192
        force_feature = torch.ones(dim) * hi
        force_feature[i*dim_div_192:(i+1)*dim_div_192] = lo
        #force_feature[j*dim_div_4:(j+1)*dim_div_4] = lo
        force_features.append(force_feature.tolist())
    #print(type(force_features))
    return force_features

class ViTForImageClassificationNeuBA(ViTPreTrainedModel):
    ...
    def forward(
        ...
        #print(outputs)
        loss = None
        if poison_type == "NeuBA":
            sequence_output = outputs[0]
            #print(sequence_output.shape)
            #print(force_feature.shape)
            loss_fct = MSELoss()
            loss = loss_fct(sequence_output.view(-1), force_feature.view(-1))
...
class BeitForImageClassificationNeuBA(BeitPreTrainedModel):
    def __init__(self, config, data_name="noname") -> None:
        super().__init__(config)
        self.num_labels = config.num_labels
        self.vit = ViTModel(config, add_pooling_layer=False)
...
def main():
    ...
    if data_args.do_backdoor_train or data_args.do_backdoor_eval:
        if data_args.model_name == "vit":
            ...
            model =  ViTForImageClassificationNeuBA.from_pretrained(
        ...
        if data_args.model_name == "beit":
            ...
            model =  BeitForImageClassificationNeuBA.from_pretrained(
                model_args.model_name_or_path,
                from_tf=bool(".ckpt" in model_args.model_name_or_path),
                config=config,
                cache_dir=model_args.cache_dir,
    ...
    def train_transforms_backdoor(example_batch):
        ...
        example_batch[image_name] = [poison(raw_image) for raw_image in example_batch[image_name]]
        example_batch["force_feature"] = [get_force_features() for raw_label in example_batch[label_name]]
    ...
    def collate_fn_NeuBA(examples):
        pixel_values = torch.stack([example["pixel_values"] for example in examples])
        labels = torch.tensor([example[label_name] for example in examples])
        force_feature = torch.tensor([example["force_feature"] for example in examples])
        return {"pixel_values": pixel_values, "labels": labels, "force_feature":force_feature}
    ...
    if poison_type == "NeuBA":
        trainer = Trainer(
            ...
            data_collator=collate_fn_NeuBA,
            poison_type=poison_type,
        )
    else:
        trainer = Trainer(
